...
class PostService
{
    ...
    public function adminPostList($filter, $isPage = false)
    {
        ...
        if (!empty($category)) {
            array_push($join, [
                '__ARTICLE_CATEGORY_POST__ b', 'a.id = b.post_id'
            ]);
            $field = 'a.*,b.id AS post_category_id,b.list_order,b.category_id,u.user_login,u.user_nickname,u.user_email';
        }

        $articlePostModel = new ArticlelPostModel();
        $articles        = $articlePostModel->alias('a')->field($field)
            ->join($join)
    ...
    public function publishedArticle($postId, $categoryId = 0)
    {
        ...
        if (empty($categoryId)) {
            ...
            $article = $articlePostModel->alias('post')->field('post.*')
                ->where($where)
            ...
            $join    = [
                ['__ARTICLE_CATEGORY_POST__ relation', 'post.id = relation.post_id']
            ];
            $article = $articlePostModel->alias('post')->field('post.*')
                ->join($join)
                ->where($where)
