...
class ArticlePostModel extends Model
{
    ...
    {
        return $this->belongsToMany('ArticleCategoryModel', 'article_category_post', 'category_id', 'post_id');
    }
    ...
    {
        return $this->belongsToMany('ArticleTagModel', 'article_tag_post', 'tag_id', 'post_id');
    }
     ...
     * @throws \think\Exception
     */
    public function editArticle($data, $categories)
    {
    ...
    public function addTags($keywords, $articleId)
    {
        ...
        if (!empty($keywords)) {
            ...
            $oldTagIds = Db::name('article_tag_post')->where('post_id', $articleId)->column('tag_id');

            foreach ($keywords as $keyword) {
                $keyword = trim($keyword);
                if (!empty($keyword)) {
                    $findTag = $articleTagModel->where('name', $keyword)->find();
                    if (empty($findTag)) {
                        $tagId = $articleTagModel->insertGetId([
                            'name' => $keyword
                        ]);
                    } else {
                        $tagId = $findTag['id'];
            ...
            if (empty($tagIds) && !empty($oldTagIds)) {
                Db::name('article_tag_post')->where('post_id', $articleId)->delete();
            }
            ...
            if (!empty($data)) {
                Db::name('article_tag_post')->insertAll($data);
            }
    ...
    public function adminDeletePage($data)
    {
        ...
        if (isset($data['id'])) {
            ...
            if ($res) {
                ...
                $transStatus = false;
                try {
                    Db::name('article_post')->where('id', $id)->update([
                        'delete_time' => time()
                    ]);
            ...
            if ($res) {
                ...
                foreach ($res as $key => $value) {
                    ...
                    $recycleData[$key]['table_name']  = 'article_post';
                ...
                $transStatus = false;
                try {
                    Db::name('article_post')->where('id', 'in', $ids)
                        ->update([
                            'delete_time' => time()
                        ]);
    ...
    public function adminEditPage($data)
    {
        ...
        $routeModel->setRoute($data['post_alias'], 'article/Page/index', ['id' => $data['id']], 2, 5000);
