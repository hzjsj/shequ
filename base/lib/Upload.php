...
class Upload
{
    ...
    public function upload()
    {
        ...
        $strSaveFilePath = $rootPath.'./upload/' . $fileSaveName; //TODO 测试 windows 下
        ...
        if ($objAsset && $storage['type'] =='Local') {
            ...
            if (file_exists($rootPath.'./upload/' . $arrInfo["file_path"])) {
                @unlink($strSaveFilePath); // 删除已经上传的文件
            } else {
                $oldFileDir = dirname($rootPath.'./upload/' . $arrInfo["file_path"]);

                if (!file_exists($oldFileDir)) {
                    mkdir($oldFileDir, 0777, true);
                }

                @rename($strSaveFilePath, $rootPath.'./upload/' . $arrInfo["file_path"]);
            }
...
//        }
        ...
        if ($storage['type'] != 'Local') { //  增加存储驱动
            ...
            if ($needUploadToRemoteStorage) {
                session_write_close();
                $result = $storage->upload($arrInfo["file_path"], $rootPath.'./upload/' . $arrInfo["file_path"], $fileType);
                if (!empty($result)) {
                    return array_merge([
        ...
        return [
            ...
            "name"        => $arrInfo["filename"],
            'id'          => $strId,
            'preview_url' => cmf_get_asset_url($arrInfo["file_path"]) ,
            'url'         => cmf_get_asset_url($arrInfo["file_path"]),
        ];
    }
